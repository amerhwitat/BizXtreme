...
document.querySelectorAll('[data-action]').forEach(b=>b.addEventListener('click',async()=>{state.screen=b.dataset.action;if(state.screen==='dashboard'){createGameDashboard(document.body,{getState:()=>state,setState:(s)=>Object.assign(state,s),notify});return}notify(messages[state.screen]||'Frontier command opened.')}));
window.addEventListener('beforeunload',()=>{saveGame(state);recordScore(state)});
setInterval(()=>saveGame(state),30000);
addEventListener('resize',()=>{camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight)});
let t=0;renderer.setAnimationLoop(time=>{t=time*.001;state.playTime=t;state.score=Math.floor(t*10);state.xp=Math.floor(t*3);ring.rotation.z=t*.35;beacon.position.y=3.5+Math.sin(t*2)*.15;bars.forEach((bar,i)=>bar.userData.tick(t+i*.3));camera.position.x=Math.sin(t*.08)*1.8;camera.lookAt(0,2,-5);renderer.render(scene,camera)});
