...
public final class TycoonGame {
    ...
    public PurchaseResult buyBusiness(String type,long price){if(price<=0||cash<price||!BUSINESSES.containsKey(type))return new PurchaseResult(false,null);cash-=price;owned.merge(type,1,Integer::sum);return new PurchaseResult(true,new Payment("ETH",PRIMARY_ETH_ADDRESS,price,"wallet-authorization-required"));}
...
record Payment(String asset,String recipient,long amount,String status){} record PurchaseResult(boolean ok,Payment payment){} record TurnResult(int turn,long revenue,long costs,long profit,long cash){}
