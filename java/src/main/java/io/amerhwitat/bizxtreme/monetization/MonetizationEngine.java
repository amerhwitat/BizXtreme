...
public final class MonetizationEngine {
    ...
    private static final PaymentRouting PAYMENT_ROUTING = new PaymentRouting(
        new PaymentTarget("ETH", "0x0B4fF3fc6AE19fAF9A0d2628a646ABD9636B1162", null),
        new PaymentTarget(null, null, "amer.hwaitat@gmail.com"),
        "ethereum", "paypal", true);
    ...
    public Result purchase(String productId, String provider) {
        ...
        if (p == null) return new Result(false, productId, provider, 0, "unknown-product", PAYMENT_ROUTING);
        events.add(new Event("purchase", productId, provider, p.price(), testMode));
        if (!p.type().equals("consumable")) entitlements.add(productId);
        return new Result(true, productId, provider, p.price(), "verification-required", PAYMENT_ROUTING);
    }
    public Result recordAdImpression(String placement, String provider) {
        events.add(new Event("ad_impression", placement, provider, 0, testMode));
        return new Result(true, placement, provider, 0, "recorded", PAYMENT_ROUTING);
    }
    public Result grantRewardedAd(String placement, String reward) {
        if (!placement.startsWith("rewarded_")) return new Result(false, placement, "", 0, "not-rewarded-placement", PAYMENT_ROUTING);
        events.add(new Event("rewarded_ad:" + reward, placement, "", 0, testMode));
        return new Result(true, placement, "", 0, "granted", PAYMENT_ROUTING);
    }
